...
def import_data_to_neo4j(driver):
    with driver.session() as session:
        # Import User Data
        session.run("LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/import/users_DEMO.csv' AS line "
        ...
        session.run("LOAD CSV WITH HEADERS FROM 'file:///var/lib/neo4j/import/recommendations_DEMO.csv' AS line "
...
if __name__ == "__main__":
    ...
    pg_cursor = pg_conn.cursor()
        ...
        #print('redis successfully executed.')
    for csv_file_path in csv_file_path_all:
        ...
        # Take one of the queries and adjust the order of predicates
        # Implement additional caching logic as needed


    result, source = query_launcher(queries[0], redis_conn, pg_cursor)




        # Connect to Neo4j
    neo4j_password = ''  # The password you set in docker-compose.yml
    neo4j_uri = "bolt://localhost:7687"
    neo4j_driver = GraphDatabase.driver(neo4j_uri, auth=("neo4j", neo4j_password))

    users_fp = './users.csv'
    recommendations_fp = './recommendations.csv'
    games_fp = './games.csv'
